...
-- this proves that ℝ is in fact an instance of a lattice by inferInstance
...
theorem absorb2 : x ⊔ x ⊓ y = x := by
  exact sup_inf_self
-- how to do these without these properties, not exactly sure how to interact with the supremum and infimum
...
example (a b c : ℚ) : (a•(![1,1,0]:Fin 3 → ℚ) + b•![1,-1,0] + c•![3,2,1] = 0) →
  ...
  intro h1 h2 h3 _
  rw[h3] at h2 h1
  rw[zero_mul] at h2 h1
  rw[add_zero] at h2 h1
  have k1: a = 0 := by
