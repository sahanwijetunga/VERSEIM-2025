...
--------------------------------------------------------------------------------
-- ring has two operations: + and * (all assoc and distr. apply)
...
#eval ((2/3)⁻¹:ℚ)
#eval (0⁻¹:ℚ)
-- lean has the convention that the inverse of zero is zero
...
#check V →ₗ[k] W
-- you get the →ₗ so, the right arrow, →, and then subscript l, \_l . So, \r\_l = →ₗ
