...
#eval (Pi.single (0:Fin 3) (1:ℚ) : Fin 3 → ℚ) (0:Fin 3)
...
theorem sbv_lin_indep' : ∀ (n : ℕ), LinearIndependent ℚ (sbv n) :=
fun n ↦
  Fintype.linearIndependent_iff.mpr fun l h ↦
    of_eq_true
      (Eq.trans
        (forall_congr fun i ↦
          Eq.trans (congrArg (fun x ↦ x i = 0) (Eq.mp (congrArg (fun _a ↦ _a = 0) (sbv_lin_comb' n l)) h)) (eq_self 0))
        (implies_true (Fin n)))
...
theorem sbv_span (n:ℕ) : Submodule.span ℚ (Set.image (sbv n) ⊤) = (⊤:Submodule ℚ (Fin n → ℚ)) := by
  ...
  . intro h
    ...
    use g
    nth_rewrite 2[this]
    refine Eq.symm (Fintype.sum_congr (fun a ↦ v a • sbv n a) (fun a ↦ v a • ↑(g a)) ?_)
    intro a
    dsimp
    refine Mathlib.Tactic.LinearCombination.smul_const_eq ?_ (v a)
    suffices Pi.single (M := fun _ ↦ ℚ) a (1: ℚ) = sbv n a from ?_
    . suffices ↑(g a)=sbv n a from ?_
      . tauto
      unfold g
      simp
    simp
    ...
    -- refine (Fintype.mem_span_set ℚ).mpr ?_
    -- have c: (⊤: (Set (Fin n))) → ℚ := fun a ↦ v a
    -- use c
    -- suffices h':(∑ i, (fun a ↦ v a) i • sbv n ↑i = v) from ?_
    -- . suffices (∑ i, (fun a ↦ v a) i • sbv n ↑i = v)=(∑ i, c i • sbv n ↑i = v) from ?_
    --   . tauto
    --   have h'': (i: (⊤: (Set (Fin n)))) -> (fun a ↦ v a) i = c i := by sorry
    --   sorry
    -- show ∑ i, v i • sbv n i = v
    -- exact this.symm
