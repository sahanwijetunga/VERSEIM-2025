...
example (α :Type) (G:Group₁ α) : ∀ x : α, G.mul x x = x → x = G.one := by
  intro x h
  let y : α := G.inv x
  have h : G.mul y (Group₁.mul G x x) = G.mul y x :=  by rw[h]
  rw [←G.mul_assoc y x x] at h
  unfold y at h
  rw [G.inv_mul_cancel x] at h
  rw [G.one_mul] at h
  assumption
...
example {α:Type} [ MyDisplay α ] (a:α) : String := by
  exact MyDisplay.mydisp a ++ MyDisplay.mydisp a
...
example (x y : G) : (x*y)⁻¹ =y⁻¹ *x⁻¹ := by group

variable (A: Type) [AddGroup A]

example (a b c: A) : a+b+c=a+(b+c) := add_assoc _ _ _
