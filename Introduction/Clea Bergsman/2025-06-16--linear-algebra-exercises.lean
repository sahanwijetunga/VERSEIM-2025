...
theorem ker_eq_of_multiple (t:k) (ht: t ≠ 0) (hfg :f = t•g) :
  ...
  intro hf
  rw [hfg] at hf
  exact non_zero_multiple t ht (g x) hf
  intro hg
  rw [hfg]
  have a : (t • g) x = t • (g x) := by rfl
  rw [a]
  rw [hg, smul_eq_mul, mul_zero]
end problem2
...
example (hpsi :  ker ψ = ⊥) : ker (ψ ∘ₗ π) = ker π := by
  ...
  constructor
  intro h
  have hz : π x ∈ ker ψ := LinearMap.mem_ker.mpr h
  rw [hpsi] at hz
  assumption
  intro h
  rw [h]
...
example (h: ker ψ = ⊥) ( k: ψ z = 0) : z=0 := by
  have hz : z ∈ ker ψ := LinearMap.mem_ker.mpr k
  rw [h] at hz
  assumption
